...
// Leitura e gravação JSON seguras usadas por configurações, cliques e outras rotas.
function idealReadJson(filePath, fallback) {
  try {
    ...
    console.error('[idealReadJson]', filePath, error.message);
    return fallback;
  }
}

function idealWriteJson(filePath, data) {
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
...
// Compatibilidade das rotas antigas de grupos.
// Mantém um único banco persistente e evita ReferenceError ao salvar.
const idealReadGroups = idealGroups;
const idealNormalizeGroupUrl = normalizeGroupLink;
function idealIncrementRegion(){ return true; }
...
http.createServer(async (req, res) => {
  ...
  if(req.method==='POST'&&url.pathname==='/api/groups/mark-posted'){
    ...
    const groups=idealGroups();
    const id=String(body.id||'').trim();
    const targetUrl=idealNormalizeGroupUrl(body.url||body.link||'');
    let g=groups.find(x=>(id&&String(x.id||'')===id)||(targetUrl&&idealNormalizeGroupUrl(x.url||x.link||'')===targetUrl));
    if(!g&&targetUrl){
      g={id:id||Date.now().toString(36)+Math.random().toString(36).slice(2),name:body.name||'Grupo Facebook',nome:body.name||'Grupo Facebook',url:targetUrl,link:targetUrl,region:body.region||body.bairro||'',bairro:body.bairro||body.region||'',status:'pendente',createdAt:new Date().toISOString(),postedAt:null};
      groups.push(g);
    }
    if(!g)return sendJson(res,404,{ok:false,message:'Grupo não encontrado.',groups});
    g.status='postado';
    g.postedAt=body.postedAt||new Date().toISOString();
    idealWriteGroups(groups);
    return sendJson(res,200,{ok:true,group:g,groups});
  }
...
if (req.method === 'GET' && url.pathname === '/api/clicks') {
  ...
  if (req.method === 'GET' && url.pathname === '/r') {
    ...
    const target = 'https://ideal-toldos-central-production.up.railway.app/';
    if (target) {
